...
const navItems: NavItem[] = [
  { icon: LayoutDashboard, label: 'Dashboard', path: '/', roles: ['municipality', 'contractor', 'field', 'ministry'] },
  { icon: ClipboardList, label: 'Task Management', path: '/tasks', roles: ['municipality', 'contractor'] },
  { icon: Users, label: 'Contractors', path: '/contractors', roles: ['municipality', 'ministry'] },
  { icon: ClipboardCheck, label: 'Field Verification', path: '/verification', roles: ['municipality', 'field'] },
  { icon: FileText, label: 'Historical Reports', path: '/reports', roles: ['municipality', 'ministry'] },
  { icon: MapPinned, label: 'Live Tracking', path: '/tracking', roles: ['municipality'] },
  { icon: BarChart3, label: 'Analytics', path: '/analytics', roles: ['municipality', 'ministry'] },
];

const roleLabels: Record<UserRole, { label: string; icon: React.ElementType }> = {
  municipality: { label: 'Municipality Officer', icon: Building2 },
  contractor: { label: 'Contractor', icon: Truck },
  field: { label: 'Field Verifier', icon: MapPin },
  ministry: { label: 'Ministry Analyst', icon: BarChart3 }
};

export const AppSidebar = () => {
  const location = useLocation();
  const { userRole, setUserRole, sidebarCollapsed, setSidebarCollapsed } = useApp();
